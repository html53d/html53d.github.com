...
(function($) {
    ...
    $.fn.layerSlider = function( options ){
        ...
        if( (typeof(options)).match('object|undefined') ){
            ...
            if( options == 'data' ){
                var lsData =  jQuery(this).data('LayerSlider');
                if( lsData ){
                    return lsData;
                }
            }else{
                    ...
                    if( lsData ){
                        ...
                        if( ( lsData.g.autoSlideshow || ( !lsData.g.autoSlideshow && lsData.g.originalAutoSlideshow ) ) && options == 'stop' ){
                            ...
                            lsData.g.curLayer.find('iframe[src*="www.youtu"], iframe[src*="player.vimeo"]').each(function(){
                                ...
                                clearTimeout(  jQuery(this).data( 'videoTimer') );
                            });
    ...
    var layerSlider = function(el, options) {
        ...
        var ls = this;
        ls.$el =  jQuery(el).addClass('ls-container');
        ...
        ls.init = function(){
            ...
            /*			
			if(  jQuery(el).find('.ls-layer').length == 1 ){
				 jQuery(el).find('.ls-layer:eq(0)').clone().appendTo(  jQuery(el) );
			}
*/
            ...
            ls.g.sliderOriginalWidth =  jQuery(el)[0].style.width;

            // Storing unique settings of layers and sublayers into object.data

            jQuery(el).find('.ls-layer, *[class*="ls-s"]').each(function(){

                if(  jQuery(this).attr('rel') ||  jQuery(this).attr('style') ){
                    if(  jQuery(this).attr('rel') ){
                        var params =  jQuery(this).attr('rel').toLowerCase().split(';');
                    }else{
                        var params =  jQuery(this).attr('style').toLowerCase().split(';');						
                    }
                    for(x=0;x<params.length;x++){
                        ...
                        jQuery(this).data( $.trim(param[0]), $.trim(param[1]) );
                    }
                }

                // NEW FEATURE v1.7 making the slider responsive - we have to use style.left instead of jQuery's .css('left') function!

                jQuery(this).data( 'originalLeft',  jQuery(this)[0].style.left );
                jQuery(this).data( 'originalTop',  jQuery(this)[0].style.top );
            });

            // NEW FEATURE v2.0 linkTo

            jQuery(el).find('*[class*="ls-linkto-"]').each(function(){
                var lClasses =  jQuery(this).attr('class').split(' ');
                for( var ll=0; ll<lClasses.length; ll++ ){
                    if( lClasses[ll].indexOf('ls-linkto-') != -1 ){
                        var linkTo = parseInt( lClasses[ll].split('ls-linkto-')[1] );
                        jQuery(this).click(function(e){
                            e.preventDefault();
                            jQuery(el).layerSlider( linkTo );
                        });
                    }
                }
            });
            ...
            ls.g.layersNum =  jQuery(el).find('.ls-layer').length;
            ...
            jQuery(el).find('iframe[src*="www.youtu"]').each(function(){
                if(  jQuery(this).parent('[class*="ls-s"]') ){

                    var iframe =  jQuery(this);

                    // Getting thumbnail
					
                    $.getJSON('http://gdata.youtube.com/feeds/api/videos/' +  jQuery(this).attr('src').split('embed/')[1].split('?')[0] + '?v=2&alt=json&callback=?', function(data) {

                        iframe.data( 'videoDuration', parseInt(data['entry']['media$group']['yt$duration']['seconds']) * 1000 );
                    });
					
                    var vpContainer =  jQuery('<div>').addClass('ls-vpcontainer').appendTo(  jQuery(this).parent() );

                    jQuery('<img>').appendTo( vpContainer ).addClass('ls-videopreview').attr('src', 'http://img.youtube.com/vi/' +  jQuery(this).attr('src').split('embed/')[1].split('?')[0] + '/' + ls.o.youtubePreview );
                    jQuery('<div>').appendTo( vpContainer ).addClass('ls-playvideo');
                    ...
                    jQuery(this).parent().css({
                        ...
                        jQuery(this).find('iframe').attr('src',  jQuery(this).find('iframe').data('videoSrc') );
                        jQuery(this).find('.ls-vpcontainer').delay(ls.g.v.d).fadeOut(ls.g.v.fo, function(){							
                    ...
                    jQuery(this).data( 'videoSrc',  jQuery(this).attr('src') + sep + 'autoplay=1' );
            ...
            jQuery(el).find('iframe[src*="player.vimeo"]').each(function(){
                if(  jQuery(this).parent('[class*="ls-s"]') ){
                    ...
                    $.getJSON('http://vimeo.com/api/v2/video/'+ (  jQuery(this).attr('src').split('video/')[1].split('?')[0] ) +'.json?callback=?', function(data){

                        jQuery('<img>').appendTo( vpContainer ).addClass('ls-videopreview').attr('src', data[0]['thumbnail_large'] );						
            ...
            ls.g.curLayer =  jQuery(el).find('.ls-layer:eq('+(ls.g.curLayerIndex-1)+')');				
            ...
            if( ls.o.yourLogo ){
                var yourLogo =  jQuery('<img>').appendTo( jQuery(el)).attr( 'src', ls.o.yourLogo ).attr('style', ls.o.yourLogoStyle );

                // NEW FEATURES v1.8 added yourLogoLink & yourLogoTarget

                if( ls.o.yourLogoLink != false ){
                    jQuery('<a>').appendTo( jQuery(el)).attr( 'href', ls.o.yourLogoLink ).attr('target', ls.o.yourLogoTarget ).css({
                        textDecoration : 'none',
                        outline : 'none'
                    }).append( yourLogo );
                }
            ...
            jQuery(el).find('.ls-layer').wrapAll('<div class="ls-inner"></div>');
            ...
            if( ls.o.navPrevNext ){
                ...
                jQuery('<a class="ls-nav-prev" href="#" />').click(function(e){
                    e.preventDefault();
                    jQuery(el).layerSlider('prev');
                }).appendTo( jQuery(el));

                jQuery('<a class="ls-nav-next" href="#" />').click(function(e){
                    e.preventDefault();
                    jQuery(el).layerSlider('next');
                }).appendTo( jQuery(el));					
            }

            if( ls.o.navStartStop || ls.o.navButtons ){
				
                jQuery('<div class="ls-bottom-nav-wrapper" />').appendTo(  jQuery(el) );

                if( ls.o.navButtons ){
					
                    jQuery('<span class="ls-bottom-slidebuttons" />').appendTo(  jQuery(el).find('.ls-bottom-nav-wrapper') );

                    for(x=1;x<ls.g.layersNum+1;x++){

                        jQuery('<a href="#"></a>').appendTo(  jQuery(el).find('.ls-bottom-slidebuttons') ).click(function(e){
                            e.preventDefault();
                            jQuery(el).layerSlider( ( jQuery(this).index() + 1) );
                        });
                    }
                    jQuery(el).find('.ls-bottom-slidebuttons a:eq('+(ls.o.firstLayer-1)+')').addClass('ls-nav-active');
                }

                if( ls.o.navStartStop ){
					
                    jQuery('<a class="ls-nav-start" href="#" />').click(function(e){
                        e.preventDefault();
                        jQuery(el).layerSlider('start');
                    }).prependTo(  jQuery(el).find('.ls-bottom-nav-wrapper') );

                    jQuery('<a class="ls-nav-stop" href="#" />').click(function(e){
                        e.preventDefault();
                        jQuery(el).layerSlider('stop');
                    }).appendTo(  jQuery(el).find('.ls-bottom-nav-wrapper') );
                }else{

                    jQuery('<span class="ls-nav-sides ls-nav-sideleft" />').prependTo(  jQuery(el).find('.ls-bottom-nav-wrapper') );
                    jQuery('<span class="ls-nav-sides ls-nav-sideright" />').appendTo(  jQuery(el).find('.ls-bottom-nav-wrapper') );						
                }
            }

            // Adding keyboard navigation if turned on and if number of layers > 1

            if( ls.o.keybNav &&  jQuery(el).find('.ls-layer').length > 1 ){
            ...
            if('ontouchstart' in window &&  jQuery(el).find('.ls-layer').length > 1 && ls.o.touchNav ){

                jQuery(el).bind('touchstart', function( e ) {
                ...
                jQuery(el).bind('touchmove', function( e ) {
                    var t = e.touches ? e.touches : e.originalEvent.touches;
                    if( t.length == 1 ){
                ...
                jQuery(el).bind('touchend',function( e ){
                    if( Math.abs( ls.g.touchStartX - ls.g.touchEndX ) > 45 ){
            ...
            if( ls.o.pauseOnHover == true &&  jQuery(el).find('.ls-layer').length > 1 ){
				
                // BUGFIX v1.6 stop was not working because of pause on hover

                jQuery(el).find('.ls-inner').hover(
                    function(){
            ...
            jQuery(el).addClass('ls-'+ls.o.skin);
            var skinStyle = ls.o.skinsPath+ls.o.skin+'/skin.css';

            if (document.createStyleSheet){
                document.createStyleSheet(skinStyle);
            }else{
                jQuery('<link rel="stylesheet" href="'+skinStyle+'" type="text/css" />').appendTo(  jQuery('head') );
            }				
            ...
            if( ls.o.animateFirstLayer == true ){
                ...
                ls.imgPreload(ls.g.curLayer,function(){
                    ls.g.curLayer.fadeIn(1000, function(){
                        ...
                        jQuery(this).addClass('ls-active');

                        // NEW FEATURE v2.0 autoPlayVideos

                        if( ls.o.autoPlayVideos ){
                            jQuery(this).delay(  jQuery(this).data('delayin') + 25 ).queue(function(){
                                jQuery(this).find('.ls-videopreview').click();
                                jQuery(this).dequeue();
                            });							
                        }
                    });
            ...
            jQuery(el).delay(150).queue(function(){
                ls.makeResponsive( ls.g.curLayer, function(){
                    return;
                }, true);
            });
        ...
        ls.timer = function(){
            ...
            var delaytime =  jQuery(el).find('.ls-active').data('slidedelay') ? parseInt(  jQuery(el).find('.ls-active').data('slidedelay') ) : ls.o.slideDelay;
        ...
        ls.change = function(num,prevnext){
            ...
            if( ls.g.pausedByVideo == true ){
                ...
                ls.g.curLayer.find('iframe[src*="www.youtu"], iframe[src*="player.vimeo"]').each(function(){
                    ...
                    jQuery(this).parent().find('.ls-vpcontainer').fadeIn(ls.g.v.fi,function(){
                        jQuery(this).parent().find('iframe').attr('src','');						
                    });
                });
            }
			
            jQuery(el).find('iframe[src*="www.youtu"], iframe[src*="player.vimeo"]').each(function(){
            ...
            ls.g.nextLayer =  jQuery(el).find('.ls-layer:eq('+(ls.g.nextLayerIndex-1)+')');
            ...
            if(  jQuery(el).find('iframe[src*="www.youtu"], iframe[src*="player.vimeo"]').length ){
                timeOut = ls.g.v.fi;
            }
        ...
        ls.imgPreload = function(layer,callback){
            ...
            if( ls.o.imgPreload ){				
                ...
                layer.find('img').each(function(){
                    preImages.push( jQuery(this).attr('src'));
                });
                ...
                layer.find('*').each(function(){
                    ...
                    if(  jQuery(this).css('background-image') != 'none' &&  jQuery(this).css('background-image').indexOf('url') != -1 ){
        ...
        ls.resizeSlider = function(){
            ...
            if(  jQuery(el).closest('.ls-wp-forceresponsive-container').length ){

                jQuery(el).closest('.ls-wp-forceresponsive-helper').css({
                    height :  jQuery(el).outerHeight(true)
                });

                jQuery(el).closest('.ls-wp-forceresponsive-container').css({
                    height :  jQuery(el).outerHeight(true)
                });
                ...
                if( ls.g.sliderOriginalWidth.split('%') != -1 ){
                    ...
                    var newWidth =  jQuery('body').width() / 100 * percentWidth - (  jQuery(el).outerWidth() -  jQuery(el).width() );
                    jQuery(el).width( newWidth );
                }
            }
        ...
        ls.animate = function(){
            ...
            ls.g.curLayer.delay( curDelay + curTime / 80 ).animate({
                ...
                jQuery(el).find('.ls-layer').removeClass('ls-active');
                jQuery(el).find('.ls-layer:eq(' + ( ls.g.curLayerIndex - 1 ) + ')').addClass('ls-active').removeClass('ls-animating');
                jQuery(el).find('.ls-bottom-slidebuttons a').removeClass('ls-nav-active');
                jQuery(el).find('.ls-bottom-slidebuttons a:eq('+( ls.g.curLayerIndex - 1 )+')').addClass('ls-nav-active');
            ...
            ls.g.curLayer.find(' > *[class*="ls-s"]').each(function(){
                ...
                var curSubSlideDir =  jQuery(this).data('slidedirection') ?  jQuery(this).data('slidedirection') : slideDirection;
                var lml, lmt;
                ...
                var curSubSlideOutDir =  jQuery(this).data('slideoutdirection') ?  jQuery(this).data('slideoutdirection') : false;
                ...
                var curSubPar = parseInt(  jQuery(this).attr('class').split('ls-s')[1] ) * curSubParMod;
				
                var curSubDelay =  jQuery(this).data('delayout') ? parseInt( jQuery(this).data('delayout')) : ls.o.delayOut;
                var curSubTime =  jQuery(this).data('durationout') ? parseInt( jQuery(this).data('durationout')) : ls.o.durationOut;
                var curSubEasing =  jQuery(this).data('easingout') ?  jQuery(this).data('easingout') : ls.o.easingOut;

                // NEW FEATURE v1.6 added fading feature to sublayers

                if( curSubSlideOutDir == 'fade' || ( !curSubSlideOutDir && curSubSlideDir == 'fade' )){
					
                    jQuery(this).delay( curSubDelay ).fadeOut(curSubTime, curSubEasing);					
                }else{
					
                    jQuery(this).stop().delay( curSubDelay ).animate({
            ...
            ls.g.nextLayer.find(' > *[class*="ls-s"]').each(function(){
                ...
                var nextSubSlideDir =  jQuery(this).data('slidedirection') ?  jQuery(this).data('slidedirection') : slideDirection;
                var lml, lmt;
                ...
                var nextSubPar = parseInt(  jQuery(this).attr('class').split('ls-s')[1] ) * nextSubParMod;

                var nextSubDelay =  jQuery(this).data('delayin') ? parseInt( jQuery(this).data('delayin')) : ls.o.delayIn;
                var nextSubTime =  jQuery(this).data('durationin') ? parseInt( jQuery(this).data('durationin')) : ls.o.durationIn;
                var nextSubEasing =  jQuery(this).data('easingin') ?  jQuery(this).data('easingin') : ls.o.easingIn;
